...
CEC <- read_csv("./Data/CEC/CEC_data.csv", col_types = cols(horizon = col_factor(levels = c("a","b", "c", "e", "o")))) %>% 
  select(-1) 
particle_size <- read_csv("./Data/Particle size distribution/size_data.csv", 
...
labs(x = "Silt and Clay content (%)", y = "SOC (%)") +
facet_wrap(. ~ horizon, scales = "free") +
  geom_smooth(method = "lm") +
  my_theme
```
