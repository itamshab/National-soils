...
ggplot(data = calculated_bulk_CN, aes(percent_C, percent_OC, color = pH)) + geom_point()
...
size_short <- select(size_fractions_calculated_values, 1, 17, 18)
...
```
```{r joined_data}
ggplot(joined_data, aes(HH_DOC_mg_g, percent_OC, alpha = pH)) + geom_point() + my_theme + labs(x = "HH extractable DOC (mg C/g soil)", y = "SOC (%)")
```
