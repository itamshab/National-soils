...
moisture_calculated_values <- moisture %>% 
  select(2:5,7,9,10,11,12,13) %>% # selecting relevant columns
...
# generating bulkd density, hygroscopic and gravimetric df only for merging and calculating moisture normalized values
...
ggplot(data = calculated_bulk_CN, aes(horizon, SOC, alpha = pH)) + 
geom_boxplot() +
geom_jitter() +
my_theme
...
CN_short <- select(calculated_bulk_CN, 1,5, 6, 14, 15)
...
joined_data <- inner_join(all_pedon_meta_data, moisture_calculated_values) 
...
joined_data <- left_join(joined_data, calculated_MBC_DOC) %>% 
  select(unique_ID, pedon_ID, horizon, everything())
```


```{r moisture correction}
# removing b horizon samples
# normalizaing MBC/DOC to gravimetric weight and other properties to oven dry weight
joined_data_moisture_corrected <- joined_data %>% 
  ...
  # insert script for correcting everything for hygroscopic moisture
                  )
...
joined_data_per_SOC <- joined_data_moisture_corrected %>% 
...
ggplot(data = joined_data_per_SOC, aes(order, HH_DOC_C)) + geom_boxplot() + geom_jitter(aes(order, HH_DOC_C, color = horizon)) +
labs(x = "Soil order", y = "HH extractable DOC (mg/g SOC)") +
my_theme

ggplot(data = joined_data_per_SOC, aes(SOC, HH_DOC_C)) + 
geom_point(aes(Fe_plus_Al, SOC, fill = pH), shape = 21, size = 2) +
labs(y = "SOC (%)", x = "HH extractable Al+Fe (mg/g)") +
my_theme

```
```{r joining data frames no B, message=FALSE, warning=FALSE}
# select only for summarizing variable and unique_ID in each data frame
ph_carbonate_short <- select(pH_carbonate_percent, 1,2, 3, 6, 8)
CN_short <- select(calculated_bulk_CN, 1,5, 6, 14, 15)
HH_short <- select(HH_calculated_values, 2, 15, 16:18)
size_short <- select(size_fractions_calculated_values, 1, 17, 18,19)
joined_data_noB <- inner_join(all_pedon_meta_data, ph_carbonate_short) 
joined_data_noB <- left_join(joined_data_noB, CN_short) 
joined_data_noB <- left_join(joined_data_noB, HH_short) 
joined_data_noB <- left_join(joined_data_noB, size_short)
joined_data_noB <- left_join(joined_data_noB, calculated_MBC_DOC) %>% 
  select(unique_ID, pedon_ID, horizon, everything())
```
