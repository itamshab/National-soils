...
missing_HWC <- read_csv("./Data/Moisture/missing_HWC.csv",col_types = cols(horizon = col_factor(levels = c("a","b", "c", "e", "o")))) %>% 
  select(1, 5) 
...
CEC <- 
  read_csv("./Data/CEC/CEC_data.csv", col_types = cols(horizon = col_factor(levels = c("a","b", "c", "e", "o")))) %>% 
  select(-1) 
```
...
# dplyr::select only for summarizing variable and unique_ID in each data frame
ph_carbonate_short <- dplyr::select(pH_carbonate_percent, 1,2, 3, 6, 8)
CN_short <- dplyr::select(calculated_bulk_CN, 1,5, 6, 14, 15)
HH_short <- dplyr::select(HH_calculated_values, 2, 15, 16:18)
size_short <- dplyr::select(size_fractions_calculated_values, 1, 17, 18,19)
