...
data_file_in <- here("raw-data/Soil respiration/national_soil_respiration_group4b.xlsx")

metadata_file_in <- here("raw-data/Soil respiration/national_soil_respiration_group4b.xlsx")

file_out <- here("data/national_soil_respiration_group4b.rds")

wide_file_out <- here("data/wide_national_soil_respiration_group4b.rds")
...
raw_data <- read_excel(data_file_in, sheet = 4, col_names = TRUE) %>% 
  drop_na('1.5') # remove blanks with no values
raw_data[raw_data<0] <- 0 # change all negative values to zero

metadata <- read_excel(data_file_in, sheet = 2, 
                       col_names = TRUE, 
                       range = "A1:K19") # change range according to sample number
  

# transform to long data structure----

long_data <- pivot_longer(raw_data, c(2:12), #edit columns to include additional dates
...
mean_data <- long_data_joined %>% 
  ...
  summarize(mean_cumu_emission = mean(cumu_emission, na.rm = TRUE),
            mean_cumu_mineralization = mean(cumu_mineralization, na.rm = TRUE),
            mean_cumu_mineralizability = mean(cumu_mineralizability, na.rm = TRUE),
            sd_cumu_emission = sd(cumu_emission, na.rm = TRUE),
            se_cumu_emission = sd_cumu_emission/sqrt(3),
            sd_cumu_mineralization = sd(cumu_mineralization, na.rm = TRUE),
            se_cumu_mineralization = sd_cumu_mineralization/sqrt(3),
            sd_cumu_mineralizability = sd(cumu_mineralizability, na.rm = TRUE),
...
joined_mean_data <- left_join(mean_data, short_metadata, by = "unique_ID") %>%
  select(-qorpak_g, -rep, -jar_ID, -soil_g, -qorpak_soil_g, -WHC) %>% 
...
min_c_plot_group4b <- ggplot(joined_mean_data, aes(Days, min_c, group = unique_ID)) + 
...
ggsave("./eda/group4b_min_c.svg", plot = min_c_plot_group4b)


min_plot_group4b <- ggplot(joined_mean_data, aes(Days, min, group = unique_ID)) + 
  geom_point(aes(fill = unique_ID), size = 4, shape = 21, alpha = 0.2) +
...
ggsave("./eda/group4b_min.svg", plot = min_plot_group4b)
