...
data_file_in <- here("raw-data/Soil respiration/national_soil_respiration_group4a.xlsx")

metadata_file_in <- here("raw-data/Soil respiration/national_soil_respiration_group4a.xlsx")

file_out <- here("data/national_soil_respiration_group4a.rds")

wide_file_out <- here("data/wide_national_soil_respiration_group4a.rds")
...
names(raw_data)[names(raw_data) == "16.899999999999999"] <- "16.89"
raw_data[raw_data<0] <- 0 # change all negative values to zero

metadata <- read_excel(data_file_in, sheet = 2, 
                       col_names = TRUE, 
                       range = "A1:K55") # change range according to sample number
...
min_c_plot_group4a <- ggplot(joined_mean_data, aes(Days, min_c, group = unique_ID)) + 
...
ggsave("./eda/group4a_min_c.svg", plot = min_c_plot_group4a)


min_plot_group4a <- ggplot(joined_mean_data, aes(Days, min, group = unique_ID)) + 
  geom_point(size = 4, shape = 21, alpha = 0.2) +
...
ggsave("./eda/group4a_min.svg", plot = min_plot_group4a)
